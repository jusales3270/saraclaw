...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SARACLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.SARACLAW_SKIP_GMAIL_WATCHER;
    process.env.SARACLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SARACLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SARACLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SARACLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
