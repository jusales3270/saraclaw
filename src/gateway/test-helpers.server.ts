...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.SARACLAW_STATE_DIR;
  previousConfigPath = process.env.SARACLAW_CONFIG_PATH;
  previousSkipBrowserControl = process.env.SARACLAW_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.SARACLAW_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.SARACLAW_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SARACLAW_STATE_DIR = path.join(tempHome, ".openclaw");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.SARACLAW_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.SARACLAW_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.SARACLAW_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.SARACLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SARACLAW_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.SARACLAW_SKIP_CANVAS_HOST;
    } else {
      process.env.SARACLAW_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.SARACLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SARACLAW_GATEWAY_TOKEN = fallbackToken;
  }
