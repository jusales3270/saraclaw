...
describe("oauth paths", () => {
  it("prefers SARACLAW_OAUTH_DIR over SARACLAW_STATE_DIR", () => {
    const env = {
      SARACLAW_OAUTH_DIR: "/custom/oauth",
  ...
  it("derives oauth path from SARACLAW_STATE_DIR when unset", () => {
    const env = {
      SARACLAW_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("state + config path candidates", () => {
  it("uses SARACLAW_STATE_DIR when set", () => {
    const env = {
      SARACLAW_STATE_DIR: "/new/state",
    } as NodeJS.ProcessEnv;
  ...
  it("CONFIG_PATH prefers existing config when present", async () => {
    ...
    const previousOpenClawConfig = process.env.SARACLAW_CONFIG_PATH;
    const previousOpenClawState = process.env.SARACLAW_STATE_DIR;
    try {
      ...
      if (previousOpenClawConfig === undefined) {
        ...
        process.env.SARACLAW_CONFIG_PATH = previousOpenClawConfig;
      }
      if (previousOpenClawConfig === undefined) {
        delete process.env.SARACLAW_CONFIG_PATH;
      } else {
      ...
      if (previousOpenClawState === undefined) {
        ...
        process.env.SARACLAW_STATE_DIR = previousOpenClawState;
      }
      if (previousOpenClawState === undefined) {
        delete process.env.SARACLAW_STATE_DIR;
      } else {
  ...
  it("respects state dir overrides when config is missing", async () => {
    ...
    try {
      ...
      const env = { SARACLAW_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
