...
describe("sessions", () => {
  ...
  it("derives session transcripts dir from SARACLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SARACLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.SARACLAW_STATE_DIR = "/custom/state";
    try {
