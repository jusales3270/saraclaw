...
export function triggerOpenClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.SARACLAW_SYSTEMD_UNIT,
        process.env.SARACLAW_PROFILE,
      );
  ...
  const label =
    process.env.SARACLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SARACLAW_PROFILE);
