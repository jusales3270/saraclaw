...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    mocks.buildServiceEnvironment.mockReturnValue({ SARACLAW_PORT: "3000" });
    ...
    expect(plan.environment).toEqual({ SARACLAW_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.SARACLAW_PORT).toBe("3000");
