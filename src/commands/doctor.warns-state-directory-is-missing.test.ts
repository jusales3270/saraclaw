...
beforeEach(() => {
  ...
  setStdinTty(true);
  originalStateDir = process.env.SARACLAW_STATE_DIR;
  originalUpdateInProgress = process.env.SARACLAW_UPDATE_IN_PROGRESS;
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.SARACLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.SARACLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.SARACLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    process.env.SARACLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when SARACLAW_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/openclaw.json",
      exists: true,
      raw: "{}",
    ...
    process.env.SARACLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
