...
const requireAgentDir = () => {
  const agentDir = process.env.SARACLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SARACLAW_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  const previousAgentDir = process.env.SARACLAW_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.SARACLAW_AGENT_DIR;
    } else {
      process.env.SARACLAW_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.SARACLAW_OAUTH_DIR;
  });

  it("writes auth-profiles.json under SARACLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-oauth-"));
    process.env.SARACLAW_STATE_DIR = tempStateDir;
    process.env.SARACLAW_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SARACLAW_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to SARACLAW_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-minimax-"));
    process.env.SARACLAW_STATE_DIR = tempStateDir;
    process.env.SARACLAW_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.SARACLAW_AGENT_DIR;
