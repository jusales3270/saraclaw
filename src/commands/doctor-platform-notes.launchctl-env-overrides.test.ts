...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "SARACLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("SARACLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv SARACLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("SARACLAW_GATEWAY_PASSWORD");
  });
