...
describe("security audit", () => {
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.SARACLAW_STATE_DIR;
    ...
    try {
      ...
      if (prevStateDir == null) {
        ...
        process.env.SARACLAW_STATE_DIR = prevStateDir;
      }
    }
  });
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    ...
    process.env.SARACLAW_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: OpenClawConfig = {
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.SARACLAW_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.SARACLAW_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.SARACLAW_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.SARACLAW_GATEWAY_PASSWORD;
      } else {
        process.env.SARACLAW_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.SARACLAW_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: OpenClawConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.SARACLAW_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: OpenClawConfig = {
        gateway: {
          mode: "remote",
