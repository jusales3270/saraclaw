...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("marks bundled skills blocked by allowlist", async () => {
    ...
    const originalBundled = process.env.SARACLAW_BUNDLED_SKILLS_DIR;
    ...
    try {
      process.env.SARACLAW_BUNDLED_SKILLS_DIR = bundledDir;
      const report = buildWorkspaceSkillStatus(workspaceDir, {
        managedSkillsDir: path.join(workspaceDir, ".managed"),
      ...
      if (originalBundled === undefined) {
        delete process.env.SARACLAW_BUNDLED_SKILLS_DIR;
      } else {
        process.env.SARACLAW_BUNDLED_SKILLS_DIR = originalBundled;
      }
    }
  });
