...
describe("resolveGatewayStateDir", () => {
  ...
  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SARACLAW_PROFILE: "rescue" };
  ...
  it("uses SARACLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SARACLAW_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in SARACLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SARACLAW_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SARACLAW_STATE_DIR: "C:\\State\\openclaw" };
