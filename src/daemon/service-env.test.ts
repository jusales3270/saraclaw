...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SARACLAW_GATEWAY_PORT).toBe("18789");
    expect(env.SARACLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.SARACLAW_SERVICE_MARKER).toBe("openclaw");
    expect(env.SARACLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SARACLAW_SERVICE_VERSION).toBe("string");
    expect(env.SARACLAW_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.SARACLAW_LAUNCHD_LABEL).toBe("ai.openclaw.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SARACLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SARACLAW_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.SARACLAW_LAUNCHD_LABEL).toBe("ai.openclaw.work");
    }
  });
});
