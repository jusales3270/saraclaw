...
declare global {
  interface Window {
    __SARACLAW_ASSISTANT_NAME__?: string;
    __SARACLAW_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__SARACLAW_ASSISTANT_NAME__,
    avatar: window.__SARACLAW_ASSISTANT_AVATAR__,
  });
}
