...
MODELS_MODE="${SARACLAW_E2E_MODELS:-${SARACLAW_E2E_MODELS:-both}}" # both|openai|anthropic
INSTALL_TAG="${SARACLAW_INSTALL_TAG:-${SARACLAW_INSTALL_TAG:-latest}}"
E2E_PREVIOUS_VERSION="${SARACLAW_INSTALL_E2E_PREVIOUS:-${SARACLAW_INSTALL_E2E_PREVIOUS:-}}"
SKIP_PREVIOUS="${SARACLAW_INSTALL_E2E_SKIP_PREVIOUS:-${SARACLAW_INSTALL_E2E_SKIP_PREVIOUS:-0}}"
OPENAI_API_KEY="${OPENAI_API_KEY:-}"
ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
ANTHROPIC_API_TOKEN="${ANTHROPIC_API_TOKEN:-}"

if [[ "$MODELS_MODE" != "both" && "$MODELS_MODE" != "openai" && "$MODELS_MODE" != "anthropic" ]]; then
  echo "ERROR: SARACLAW_E2E_MODELS must be one of: both|openai|anthropic" >&2
  exit 2
fi

if [[ "$MODELS_MODE" == "both" ]]; then
  if [[ -z "$OPENAI_API_KEY" ]]; then
    echo "ERROR: SARACLAW_E2E_MODELS=both requires OPENAI_API_KEY." >&2
    exit 2
  fi
  if [[ -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
    echo "ERROR: SARACLAW_E2E_MODELS=both requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
    exit 2
  fi
elif [[ "$MODELS_MODE" == "openai" && -z "$OPENAI_API_KEY" ]]; then
  echo "ERROR: SARACLAW_E2E_MODELS=openai requires OPENAI_API_KEY." >&2
  exit 2
elif [[ "$MODELS_MODE" == "anthropic" && -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
  echo "ERROR: SARACLAW_E2E_MODELS=anthropic requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
  exit 2
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (SARACLAW_INSTALL_E2E_SKIP_PREVIOUS=1)"
else
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  SARACLAW_BETA=1 SARACLAW_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  SARACLAW_VERSION="$INSTALL_TAG" SARACLAW_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
